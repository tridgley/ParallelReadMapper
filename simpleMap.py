"""
 ...
 simpleMap.py -- Performs DNA sequence alignments with a sequential algorithm
    ...
    def getMatches(self, searchString):
        ...
        for i in range(len(searchString)-self.w+1):
            ...
            if sites[0] not in yieldedSites and self.minimizerMap.get(minmer):
                # print(sites[0], self.minimizerMap[minmer])
                yieldedSites.append(sites[0])
                # minmerOccurrences.append((sites, self.minimizerMap[minmer]))
...
def simpleMap(targetString, minimizerIndex, queryString, config):
    ...
    =====> The slowest part of this mapping is the SmithWaterman alignment, which is
           exacerbated by running the reverse compliment as well. If we knew which strand
           the query read was from, then we could just determine which forward or 
           reverse strand to query a single time. Also, we are running this code sequentially
           for 500 query strings, but I was thinking that we could get a bulk set of
           seeds from all of the queries and run getMatches once, then we could
           build the connected components a single time and associate them with the original
           reads. Then, only connected components of a certain size could be sent to
           SmithWaterman for alignment since we are probably only looking for good
           scoring query sequences.
    ...
    def mapForwards(queryString):
        ...
        for seedCluster in SeedCluster.clusterSeeds(list(seeds), l=config.l):
            ...
            # print("SmithWatermanInfo:", len(seeds), len(querySubstring), len(targetSubstring))
